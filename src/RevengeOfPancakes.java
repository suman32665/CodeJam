...
/**
 * Created by suman maharjan on 16/02/2017.
 Problem
 ...
 the stack, flip that entire group over, and then put the group back down on top of any pancakes that you did not lift up.
 ...
 Then, after the flip, the stack is i, i-1, ..., 2, 1, i+1, i+2, ..., N. Pancakes 1, 2, ..., i now have the opposite side up, w
 hereas pancakes i+1, i+2, ..., N have the same side up that they had up before.
 ...
 them back down on the remaining fourth pancake (which would stay where it is and remain unchanged). The new state of the
 stack would then be -++-. The other valid ways would be to pick up and flip the top one, the top two, or all four. It would
 not be valid to choose and flip the middle two or the bottom one, for example; you can only take some number off the top.
...
public class RevengeOfPancakes {
    public static void main(String[] args) {
        ...
        if (checkTestCase_size(testCases)){
            for (int i=0; i<testCases;i++){
                input=input_string();
                if (checkInputSize(input.length())){
                    inputSet.add(input);
                }
            }
            computation(testCases,inputSet);
        }
    }
    public static void computation(int testCases, List inputSet)
    {
        String input;
        int count;
        for (int i=0; i<testCases;i++){
            input=inputSet.get(i).toString();
            count=0;
            for (int j=input.length()-1;j>=0;j--){
                if (input.charAt(j)=='-'){
                    String subInput1=flip_pancakes(input.substring(0,j+1));
                    String subInput2=input.substring(j+1,input.length());
                    input=subInput1+subInput2;
                    count++;
                }
                if (positivePancakes(input)){
                    int k=i+1;
    ...
    public static boolean positivePancakes(String pancakes)
    {
    ...
    public static String flip_pancakes(String pancakes)
    {
    ...
    public static boolean checkInputSize(int size)
    {
        if (size>=1 && size<=100){
            return true;
        }else{
    ...
    public static boolean checkTestCase_size(int testCases)
    {
